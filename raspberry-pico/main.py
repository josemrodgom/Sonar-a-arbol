...
# Slightly modified by Jose Maria Rodriguez for Sonar a arbol project
...
async def send_task():
    ...
    sensor_flex1 = machine.ADC(26)
    sensor_flex2 = machine.ADC(27)
    sensor_flex3 = machine.ADC(28)
    
    while True:
        led.toggle()
        blink = 10 if connected else 250
        if connected:
            #today = time.localtime()  # get the current time
            reading1 = sensor_flex1.read_u16() * conversion_factor
            reading2 = sensor_flex2.read_u16() * conversion_factor
            reading3 = sensor_flex3.read_u16() * conversion_factor
            message = f"{reading1} {reading2} {reading3}"
